...
Requires the graphviz library (https://www.graphviz.org/) and python 3

Forked from [rod2ik/mkdocs-graphviz](https://gitlab.com/rod2ik/mkdocs-graphviz)
"""
...
DEFAULT_LIGHT_THEME_COLOR = '000000'
DEFAULT_DARK_THEME_COLOR = 'FFFFFF'

DEFAULT_COLOR = DEFAULT_COLOR.lower()
DEFAULT_LIGHT_THEME_COLOR = DEFAULT_LIGHT_THEME_COLOR.lower()
DEFAULT_DARK_THEME_COLOR = DEFAULT_DARK_THEME_COLOR.lower()
DEFAULT_PRIORITY = '75'
DEFAULT_DOT_OUTPUT_FORMAT = 'png'
...
class MkdocsGraphvizExtension(markdown.Extension):
    def __init__(self, **kwargs):
        self.config = {
            ...
            'light_theme': [DEFAULT_LIGHT_THEME_COLOR, 'Default Light Color for Nodes & Edges'],
            'dark_theme': [DEFAULT_DARK_THEME_COLOR, 'Default Dark color for Nodes & Edges'],
            ...
            'dot_output_format': [DEFAULT_DOT_OUTPUT_FORMAT, 'Default output format for dot command'],
        }
    ...
    def extendMarkdown(self, md):
        ...
        priority = int(self.config['priority'][0])
        md.preprocessors.register(MkdocsGraphvizPreprocessor(md, self.config), 'graphviz_block', priority)


def read_block(text: str) -> (str, int) or (None, -1):
    """Returns a tuple:
    ...
    blocks = [BLOCK_RE_GRAPHVIZ.search(text), BLOCK_RE_DOT.search(text)]
...
def get_decalage(command: str, text: str) -> int:
    """
    Renvoie le décalage (nombre d'espaces) où commencent les ``` dans la ligne ```command ...
    Cela suppose que le 'text' réellement la commande, ce qui est censé être le cas lros de l'utilisation
    de cette fonction
    """
...
def escape_chars(output):
    for c in ESC_CHAR:
        output = output.replace(c, ESC_CHAR[c])
    return output


class MkdocsGraphvizPreprocessor(Preprocessor):
    ...
    def set_html_colors(self):
        ...
        for colorKey in color_dict:
            ...
            if self.config[colorKey][0] in HTML_COLORS.keys():
                ...
                if colorKey not in ['priority', 'dot_output_format']:
    ...
    def repair_broken_svg_in(self, output):
        ...
        lines = output.split("\n")
        new_lines = []
        search_text = "Generated by graphviz"
        for i in range(len(lines)):
            if i + 3 <= len(lines) - 1 and any(search_text in lines[j] for j in range(i - 1, i + 4)):
                continue
            if i >= 3 and ("<svg" in lines[i - 1] and search_text in lines[i - 4]):
                continue
            if i >= 3 and ("<svg" in lines[i] and search_text in lines[i - 3]):
                s = lines[i] + lines[i + 1]
                s = s[:s.index(">")] + """ class="graphviz dot kingen-test">"""
                new_lines.append(s)
            else:
                new_lines.append(lines[i])
        new_lines = new_lines[1:]
        new_output = "\n".join(new_lines)
        xml_headers = f"""<span class="graphviz-light-dark" data-library-default="#{DEFAULT_COLOR}" data-default="{self.config['color'][0]}" data-light="{self.config['light_theme'][0]}" data-dark="{self.config['dark_theme'][0]}"></span>"""
        # xmlHeaders += f"""<?xml version="1.0" encoding="UTF-8" standalone="no"?>"""
        # xmlHeaders += f"""<!-- Generated by graphviz {graphvizVersion} -->"""
        new_output = xml_headers + new_output
        ...
        while 1:
            ...
            if not m:
                ...
                if block_type == GRAPHVIZ_COMMAND:  # General Graphviz command
                    ...
                    decalage = get_decalage("graphviz " + command, text)
                else:  # DOT command
                    # text = self.escape_chars(text)
                    filename = "noname." + self.config['dot_output_format'][0]
                    command = "dot"
                    decalage = get_decalage(command, text)
                ...
                try:
                    ...
                    if self.config['bgcolor'][0] == 'None' or self.config['bgcolor'][0] == 'none':
                        args = [command, '-Gbgcolor=none', f'-Gcolor={graph_color}', f'-Gfontcolor={graph_fontcolor}',
                                f'-Ncolor={node_color}', f'-Nfontcolor={node_fontcolor}', f'-Ecolor={edge_color}',
                                f'-Efontcolor={edge_fontcolor}', '-T' + filetype]
                    else:
                        args = [command, f'-Gcolor={graph_color}', f'-Gfontcolor={graph_fontcolor}',
                                f'-Gbgcolor={bgcolor}', f'-Ncolor={node_color}', f'-Nfontcolor={node_fontcolor}',
                                f'-Ecolor={edge_color}', f'-Efontcolor={edge_fontcolor}', '-T' + filetype]
...
def makeExtension(**kwargs):
    return MkdocsGraphvizExtension(**kwargs)


if __name__ == '__main__':
    ext = MkdocsGraphvizExtension()
    proc = MkdocsGraphvizPreprocessor(Markdown(), ext.config)
    with open(r'D:\Projects\wiki-kingen\docs\py\lib\mkdocs.md', 'r', encoding='utf-8') as fp:
        md_lines = fp.readlines()
    print(proc.run(md_lines))
